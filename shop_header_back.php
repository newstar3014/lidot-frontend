...
<style media="screen">
	...
	@media (max-width:767px) {
		#kakao-talk-channel-chat-button, #kakao-talk-channel-chat-button-m {
			position: fixed;
			z-index: 999;
			right: 15px; /* 화면 오른쪽으로부터의 거리, 원하시는대로 숫자를 변경하세요 */
			bottom: 30px; /* 화면 오른쪽으로부터의 거리, 원하시는대로 숫자를 변경하세요 */
		}
		#scrollUp{
			display: none !important;
		}
...
<body>
	<div class="head_Box">
		<header>
			...
			<ul class="flexcenter">
				<li><span class="loginText d-none" onclick="location.href='/login.php'">LOGIN</span></li>
				<li class="login_none joinText d-none"><span onclick="location.href='/registerType.php'">JOIN</span></li>
				<li class="login_none2 d-none"><span onclick="gomypage()">MYPAGE</span></li>
				<li class="login_none2 d-none cart-area pr-1"><div class="cart--btn" onclick="location.href='/cart.php'"><i class="fas fa-shopping-cart"></i> <span class="cart_quantity">0</span></div></li>
				...
				<li class="search_list_wrap pl-0">
					<div class="search-form d-flex align-items-center" id="search-wrap" style="position:relative;">
						<input type="search" class="form-control bg-0 w-1px border-0 opa-0" placeholder="검색어입력" id="sk" onkeyup="enterkey()" style="border-radius:1.25rem; padding-left:20px; transition:0.4s;">
						<div class="search-btn form-control col-2 text-center bg-0 border-0"  style="position:absolute; top:0; right:0; border-radius:0 1.25rem 1.25rem 0; transition:0.4s;"><i class="fas fa-search pointer" onclick="Search();"></i></div>
					</div>
				</li>
		...
		<div class="container px-0">
			<div class="pl-3 classy-nav-container breakpoint-off">
				<nav class="classy-navbar" id="bigshopNav">
					...
					<!-- Nav Brand -->
					<a href="/" class="nav-brand d-none" id="m_he_img"><img src="img/page/header/logo01.png" alt="logo" style="width: 45px;"></a>
					...
					<div class="d-flex align-items-center">
						...
						<div class="classy-menu ml-auto" id="menu-M-wrap">
							...
							<div class="classycloseIcon" style="width:90%; overflow:hidden;">
								...
								<div class="cross-wrap d-inline-block"><span class="top"></span><span class="bottom"></span></div>
							</div>
							...
							<div class="classynav" id="menu-M">
								<ul>
									...
									<div class="d-flex align-items-center justify-content-center py-2 px-4">
										<img src="/img/login_icon.png" alt="" style="width:30%;" class="login_none" onclick="location.href='login.php'">
										<img src="/img/signup_icon.png" alt="" style="width:30%;" class="login_none" onclick="location.href='register.php'">
										<img src="/img/mypage_icon.png" alt="" style="width:30%;"class="login_none2" onclick="location.href='mypage.php'">
									</div>
						...
						<div class="hero_meta_area ml-md-5 d-flex align-items-center pl-3 justify-content-end">
							...
							<div class="wishlist-area">
								<div class="login_none position-relative mr-3" onclick="location.href='/login.php'">LOGIN</div>
							</div>

							<div class="wishlist-area">
								<div class="login_none position-relative mr-3" onclick="location.href='/registerType.php'">JOIN</div>
							</div>
							<? if(isset($_SESSION['user_info'])): ?>
							<!-- Wishlist -->
							<div class="wishlist-area">
								<div class="wishlist-btn position-relative" onclick="location.href='/wishlist.php'"><i class="fas fa-heart mr-0"></i><span class="wish_quantity">0</span></div>
							</div>

							<!-- Cart -->
							<div class="cart-area">
								<div class="cart--btn" onclick="location.href='/cart.php'"><i class="fas fa-shopping-cart"></i> <span class="cart_quantity">0</span></div>
							...
							<div class="account-area pr-2 d-none">
								...
								<ul class="user-meta-dropdown">
									...
									<li class=""><a class="loginChange" href="login.php"><i class="icofont-logout"></i>
										<spna class="loginText">Login</span></a>
									</li>
									<li class="login mt-2" id="kakaoOut">
										<span class="pointer font-weight-bold" onclick="unlinkApp()" style="color:#888">카카오계정 탈퇴</span>
									</li>
								</ul>
							</div>
						</div>
...
<script type="text/javascript">
	...
	$(function(){
		...
		if(img_cookies){
			...
			for(let i=0; i<img_cookies.length; i++){
				let imgStr = `<div class="img_list pointer border my-1" onclick="location.href='/product_detail.php?p_seq=${img_cookies[i].p_seq}'" style="background-image: url('${img_cookies[i].img}')"></div>`;
	...
	function popupLoad(){
		$.ajax({
			...
			success: function(data){
				$.each(data,function(i,v){
					var str = `<div class="img_list img_list_${v.b_seq}" style="background-image: url('${v.b_img}')"></div>`;
	...
	if($(window).width() > 768){
		...
		$("#kakao-talk-channel-chat-button-m").find('img').css({"width":"40px"})
	}
	...
	function LargeSort(){
	    $.ajax({
	        ...
	        success: function(data){
				var str = `<div class="p-3 py-3 pl-4 menu-category-menu-menu menu_btns"><a href="/store.php?active=ALL">ALL</a></div>`;
				...
				var menustr = `<div class="d-inline-block py-2 px-4 af-show menu_btns d-none" style="font-size:15px;"><a style="color:black;" href="exhibition.php?active=EVENT" class=" hover_text">EVENT</a></div>
	  ...
	  $.ajax({
		  ...
		  success: function(data){
			  ...
			  if(data.length == 0){
				  ...
				  $.each(data,function(i,v){
					  var str = '<div class="pl-3"><a href="/store.php?ls_seq='+ls_seq+'&&ms_seq='+v.ms_seq+'">'+v.ms_name+'</a></div>';
