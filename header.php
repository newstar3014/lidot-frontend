    ...
    <div class="head_Box">
        <header>
            ...
            <ul class="flexcenter">
                ...
                <li class="login_none joinText d-none"><span onclick="location.href='/registerType.php'">JOIN</span>
                </li>
    ...
    $(function() {
        ...
        if ($(window).width() < 950) {
            ...
            var str = `<div class="search-form d-flex align-items-center">
							<input type="search" class="form-control form-control-sm" placeholder="검색어를 입력해보세요" id="sk" onkeyup="enterkey()">
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data, function(i, v) {
                    if (i == 0) {
                        var str =
                            `<div class="py-2 pl-2 pr-4 d-inline-block menu_btns"><a style="font-size:15px; color:black" class="pointer hover_text" onclick="sortBtn(${v.st_seq}, '${v.st_name}')">${v.st_name}</a></div>`;

                    } else {
                        var str =
                            `<div class="py-2 px-4 d-inline-block menu_btns"><a style="font-size:15px; color:black" class="pointer hover_text" onclick="sortBtn(${v.st_seq}, '${v.st_name}')">${v.st_name}</a></div>`;
                    }
    ...
    function MiddleSort(ls_seq, ls_name) {
        ...
        $.ajax({
            ...
            success: function(data) {
                $.each(data, function(i, v) {
                    var str =
                        `<div class="p-3 py-3 pl-4"><a href="/store.php?ls_seq=${ls_seq}&&ms_seq=${v.ms_seq}&&active=${ls_name}">${v.ms_name}</a></div>`;
        ...
        $.ajax({
            ...
            success: function(data) {
                $.each(data, function(i, v) {
                    ...
                    var str = `<a href="/store.php?ls_seq=${v.ls_seq}&active=${v.ls_name}">
